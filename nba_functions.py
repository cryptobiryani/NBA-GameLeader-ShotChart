...
def get_scorer_df(games_dict, game_code, is_home=True):
    ...
    for game in games_dict:
        if game['gameStatusText'] == 'Final' and game['gameCode'] == game_code:
            scoring_leader=game['gameLeaders'][leader_index]['personId']
            team=game[team_index]['teamId']
            data=shotchartdetail.ShotChartDetail(player_id=scoring_leader, last_n_games=1, team_id=team).get_dict()['resultSets'][0]
            rows=data['rowSet']
...
def get_team_acroynm(scorer_df, is_home=True):
    '''
